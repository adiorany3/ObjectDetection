...
# Muat model YOLOv8 (menggunakan cache agar tidak dimuat ulang setiap interaksi)
@st.cache_resource
def load_model():
...
st.sidebar.markdown("---")
st.sidebar.info("Model yang digunakan: **YOLOv8 Nano**\n\nDikembangkan oleh Ultralytics.")

# Tab untuk Gambar dan Video
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 Deteksi Objek pada Gambar", type="primary"):
            with st.spinner("Sedang memproses gambar..."):
                ...
                # Konversi BGR ke RGB agar bisa ditampilkan dengan benar di Streamlit
                res_plotted_rgb = cv2.cvtColor(res_plotted, cv2.COLOR_BGR2RGB)
                
                with col2:
                    st.image(res_plotted_rgb, caption="Hasil Deteksi", use_container_width=True)
                
                # Tampilkan detail objek yang terdeteksi
...
with tab2:
    ...
    if uploaded_video is not None:
        ...
        if st.button("🎬 Deteksi Objek pada Video", type="primary"):
            with st.spinner("Sedang memproses video (ini mungkin memakan waktu beberapa saat)..."):
